...
// Code modified by Wes Jones (jonesee@aol.com)
// Altered code to include serial port commands to enable cube to be turned and flipped thtough Windows
// Other code changes made to support Kitables cube solver--changes are commented in code
#include <Servo.h>
...
int move_speed = 6; //was 5
int buffer_time = 75; // time between moves
int rotate_pos = 90;
int push_pos = 47; // was 140--reversed due to Kitables machine having push servo mounted on opposite side of push arm
int hold_progress = 3;
	...
	if (num_of_pushes == 1)
		{
			if (slow_push == false)
			{
				move_servo(push_pos, 108, 6); //was 72
				delay(buffer_time);
				release_cube();
				delay(buffer_time);
			}
	...
	else
	{
		while (num_of_pushes != 0)
		{
			if (slow_push == false)
			{
				...
				move_servo(push_pos, 60, 6); //was 120
				delay(buffer_time);
				num_of_pushes--;
			}
			else // on rotate one
...
void hold_cube()
{
	move_servo(push_pos, 67, 6); //was 117
	hold_progress = 1;
}
void release_cube()
{
	move_servo(push_pos, 48, 6); //was 140
	hold_progress = 3;
}
void rotate_one()
{
	...
	if (hold_progress == 1) // hold progress 1 = hold
	{
		...
		if (rotate_pos < 140)
		{
			...
			release_cube();
			move_servo(rotate_pos, 95, 9); //was 101 ****************************************11111111111111
			hold_cube();
			move_servo(rotate_pos, 76, 9); //was 76
			move_servo(rotate_pos, 97, 9); // prevent pulling-was 92***
			release_cube();
		...
		else if (rotate_pos > 140)
		{
			...
			// release and turn some more
			release_cube();
			move_servo(rotate_pos, 95, 9);  //was 108******************************************
			hold_cube();
			move_servo(rotate_pos, 75, 9); //was 83
			move_servo(rotate_pos, 97, 9); // prevent pulling-was 93
			release_cube();
			move_servo(rotate_pos, rotate_finish, 9);
		}
...
void rotate_two()
{
	...
	if (hold_progress == 1) // hold progress 1 = hold
	{
		...
		if (rotate_pos < 50) 
		{
			...
			release_cube();
			move_servo(rotate_pos, 8, 9); //was 0
			hold_cube();
			move_servo(rotate_pos, 28, 9);//was 18
			move_servo(rotate_pos, 6, 9); // prevent pulling--was 8
			release_cube();
		...
		else if (rotate_pos > 150) 
		{
			...
			// release and turn some more
			release_cube();
			move_servo(rotate_pos, 180, 9);   //This is okay
			hold_cube();
			move_servo(rotate_pos, 170, 9); //was 170---160
			move_servo(rotate_pos, 175, 9); // prevent pulling 178------
			release_cube();
			move_servo(rotate_pos, rotate_finish, 9);
		}
		hold_progress = 2;
...
void rotate_three()
{
	...
	if (hold_progress == 1) // hold progress 1 = hold
	{
		...
		if (rotate_pos > 40)
		{
			...
			hold_cube();
			move_servo(rotate_pos, 107, 9); //was 100 
			move_servo(rotate_pos, 83, 9); // prevent pulling -was 90
			release_cube();
			move_servo(rotate_pos, rotate_finish, 9);
		}
		...
		if (rotate_pos < 40)
		{
			...
			// fix: cube not fully turned
			release_cube();
			move_servo(rotate_pos, 80, 9); //was 70
			hold_cube();
			move_servo(rotate_pos, 107, 9); //was 100
			move_servo(rotate_pos, 83, 9); // prevent pulling--was 90
			release_cube();
...
/////////////// Loop //////////////////
//Serial port code added to enable cube to be turned and flipped through Windows program
void loop()
{
  ...
  {
    cmd = Serial.readStringUntil(char(35)); // read input string until "#" is reached                        
    if (cmd=="1") 
    {
      //Spin();
      //rotate_one();
      move_servo(rotate_pos, 88, 9);
    }
    else if (cmd=="2")
    {
      //rotate_two();
      ...
      {
     // rotate_three();
      move_servo(rotate_pos, 13, 9);    
    }
    else if (cmd=="P")
    {
      push_cube();
  }
  else if (cmd=="Go") //Accept cube color string and proceed to solve cube
  {
   import_cube_colors(); 
   solve_cube();
	Serial.println("Done!");
